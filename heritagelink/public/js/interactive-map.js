...
function initMap() {
    map = L.map('map', {
        ...
        zoom: 14,
        zoomControl: false,
        maxBounds: GLORIA_BOUNDARY_COORDS,
    ...
    // Add Gloria boundary visualization
    addGloriaBoundary();
    ...
    // Fit map to Gloria bounds initially
    map.fitBounds(GLORIA_BOUNDARY_COORDS, { padding: [20, 20] });
}

// Add Gloria boundary visualization
function addGloriaBoundary() {
    // Draw Gloria boundary rectangle
    gloriaBoundary = L.rectangle(GLORIA_BOUNDARY_COORDS, {
        color: '#16a34a',
        weight: 3,
        fillColor: '#22c55e',
        fillOpacity: 0.1,
    ...
    // Add Gloria label
    const centerLat = (GLORIA_BOUNDS.north + GLORIA_BOUNDS.south) / 2;
    const centerLng = (GLORIA_BOUNDS.east + GLORIA_BOUNDS.west) / 2;
    
    L.marker([GLORIA_BOUNDS.north - 0.015, centerLng], {
        icon: L.divIcon({
            className: 'gloria-boundary-label',
            html: `<div style="
...
// Check if coordinates are within Gloria bounds
...
function createPopupContent(place, config) {
    const imageHtml = place.image_url ? 
        `<div class="popup-image" style="margin-bottom: 0.75rem;">
            <img src="${place.image_url}" alt="${escapeHtml(place.name)}" 
    ...
    return `
        <div class="info-window">
            ...
            <div style="display: flex; gap: 0.5rem; margin-top: 0.75rem;">
                <button class="btn-sm btn-green" style="flex: 1; padding: 0.6rem; font-weight: 600; background: #4285f4; border: none; border-radius: 6px; color: white; box-shadow: 0 2px 4px rgba(66,133,244,0.3)" onclick="getDirectionsTo(${place.latitude}, ${place.longitude}, '${escapeHtml(place.name).replace(/'/g, "\\'")}')">
                    <i class="fas fa-directions"></i> Directions
                </button>
                <button class="btn-sm btn-orange" style="flex: 1; padding: 0.6rem; font-weight: 600; background: #ff6b35; border: none; border-radius: 6px; color: white; box-shadow: 0 2px 4px rgba(255,107,53,0.3)" onclick="window.open('/places/${place.id}', '_blank')">
...
function getDirectionsTo(lat, lng, name) {
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            ...
            // Check if user is within Gloria bounds
            if (!isWithinGloria(userLat, userLng)) {
                alert('You are currently outside Gloria, Oriental Mindoro. Directions are only available within Gloria area. Please visit Gloria to use navigation features.');
                return;
            }
            ...
            userMarker.bindPopup('<strong><i class="fas fa-crosshairs"></i> Your Location</strong><br>Gloria, Oriental Mindoro');
...
async function getRoadRoute(startLng, startLat, endLng, endLat) {
    try {
        // Using OpenRouteService public API (free tier)
        ...
        // Try alternative routing service (OSRM - Open Source Routing Machine)
        try {
...
function findNearby() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            // Check if user is within Gloria bounds
            if (!isWithinGloria(userLat, userLng)) {
                alert('You are currently outside Gloria, Oriental Mindoro. This map only shows places within Gloria area. Please visit Gloria to use location-based features.');
                // Still show the map centered on Gloria
                map.setView(GLORIA_CENTER, 14);
                return;
            }
            ...
            userMarker.bindPopup('<strong>Your Location</strong><br>Welcome to Gloria, Oriental Mindoro!').openPopup();
