...
router.get('/destinations', async (req, res) => {
  try {
      ...
      res.render('destinations', { 
        title: 'Destinations & Map - HeritageLink',
        user: req.session.user,
        places,
        routes,
      ...
      console.error('Failed to load map data for destinations:', err);
      res.render('destinations', { 
        title: 'Destinations & Map - HeritageLink',
        user: req.session.user,
        places: [],
...
router.get('/artisan/orders', isArtisan, async (req, res) => {
    try {
        ...
        // Fetch orders for products owned by this artisan
    ...
    try {
        ...
        try {
            const [orderRows] = await db.query(`
                ...
                LIMIT 5
            `, [req.session.user.id]);
            userOrders = orderRows || [];
        } catch (err) {
            console.log('User orders not available:', err.message);
        }
...
router.get('/user/orders', async (req, res) => {
    ...
    try {
        const [orders] = await db.query(`
            SELECT o.*, p.name as product_name, p.product_image
            FROM orders o
            JOIN products p ON o.product_id = p.id
            WHERE o.user_id = ?
            ORDER BY o.created_at DESC
...
router.get('/api/admin/stats', async (req, res) => {
    try {
        // Users by role
        let roleCounts = { user: 0, artisan: 0, admin: 0 };
        try {
            const [userRows] = await db.query('SELECT role, COUNT(*) as cnt FROM users GROUP BY role');
            userRows.forEach(r => { roleCounts[r.role] = Number(r.cnt) || 0; });
        } catch (e) { console.error('admin/stats users error:', e.message); }

        // Count helpers
        async function count(table, col, val) {
            try {
                const [rows] = await db.query(`SELECT COUNT(*) as cnt FROM ${table} WHERE ${col} = ?`, [val]);
                return rows && rows[0] ? Number(rows[0].cnt) || 0 : 0;
            } catch (e) {
                console.error(`admin/stats ${table} error:`, e.message);
                return 0;
            }
        }

        const [totalDestinations, totalEvents, pendingReviews, pendingProducts, pendingWorkshops] = await Promise.all([
            count('destinations', 'status', 'active'),
            count('events', 'status', 'active'),
            count('reviews', 'status', 'pending'),
            count('products', 'status', 'pending'),
            count('workshops', 'status', 'pending'),
        ]);

        res.json({
            success: true,
            totalUsers: roleCounts['user'],
            totalArtisans: roleCounts['artisan'],
            totalDestinations,
            totalEvents,
            pendingReviews,
            pendingProducts,
...
router.post('/api/products/:id/buy', async (req, res) => {
    try {
        ...
        const pool = await getDB();
        // Insert order with created_at set to now and user_id if logged in
        const userId = (req.session.user && req.session.user.id) ? req.session.user.id : null;
        const [result] = await pool.execute(
            'INSERT INTO orders (product_id, user_name, user_email, quantity, total_price, status, created_at, user_id) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)', 
            [prodId, name, email, qty, total, 'pending', userId]
        );
...
// Update order status
router.post('/api/orders/:id/status', isArtisan, async (req, res) => {
    try {
        ...
        const pool = await getDB();
        // Verify this order belongs to a product owned by this artisan
        const [orders] = await pool.execute(
            'SELECT o.id FROM orders o JOIN products p ON o.product_id = p.id WHERE o.id = ? AND p.artisan_id = ?',
